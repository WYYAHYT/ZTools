...
| `specs/HOST_VERTICAL_SLICE.md` | Gate 2 实现前 | accepted | validation-pending | zhangchonghao | zhangchonghao | 当前 Host Search 切片代码与自动化证据已完成，基础用户可见行为已手动复测；真实 GNOME 用户会话、Windows/macOS 真实桌面、多显示器和人工辅助技术复查仍待完成。原生全局快捷键、真实应用启动、文件/剪贴板搜索和第三方插件明确不属于本切片，不能作为当前待验证功能 |
