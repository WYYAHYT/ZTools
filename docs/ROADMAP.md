...
本路线图已经接受。各阶段依赖的设计、负责人和阻断关系统一登记在 [OPEN_ITEMS.md](OPEN_ITEMS.md)；纯技术 Gate 转换由获得委托的开发 agent 根据证据记录，涉及产品方向、公开支持承诺、费用、外部账号/服务、扩大权限或不可逆影响时由维护者授权。
...
当前首个 Host Search 切片的代码与自动化验证已完成；本阶段剩余工作只追踪已实现能力的真实平台和人工体验证据。全局快捷键、真实应用启动、文件/剪贴板搜索和第三方插件不是该切片的隐藏待办功能，分别属于后续功能切片或其他 Gate。
...
- Gate 转换必须在评审记录中明确决定，不能由提交存在、CI 通过或路线图措辞自动推断。纯技术转换可由开发 agent 按委托权完成；产品、费用、外部服务、权限或不可逆影响仍需维护者批准。
